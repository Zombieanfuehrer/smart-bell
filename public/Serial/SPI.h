...
namespace serial {
enum class SPI_mode : uint8_t { kMaster = (1 << MSTR), kSlave = (0 << MSTR) };

enum class SPI_data_order : uint8_t { kMsb_first = (0 << DORD), kLsb_first = (1 << DORD) };

enum class SPI_clock_polarity : uint8_t { kIdle_low = (0 << CPOL), kIdle_high = (1 << CPOL) };

enum class SPI_clock_phase : uint8_t { kLeading = (0 << CPHA), kTrailing = (1 << CPHA) };
