...
ConfigManager::ConfigManager(serial::UART* uart) : uart_(uart) { load_defaults(); }

void ConfigManager::load_defaults() {
  // MQTT defaults (using IP directly, not hostname string - saves RAM!)
  config_.mqtt_broker_ip[0] = 192;
  config_.mqtt_broker_ip[1] = 168;
  config_.mqtt_broker_ip[2] = 1;
  config_.mqtt_broker_ip[3] = 100;
  config_.mqtt_port = 1883;
  strncpy(config_.mqtt_client_id, "smart-bell", 23);
  config_.mqtt_client_id[23] = '\0';
  config_.mqtt_username[0] = '\0';  // Empty by default
  config_.mqtt_password[0] = '\0';  // Empty by default
  strncpy(config_.mqtt_topic_ring, "home/bell/ring", 31);
  config_.mqtt_topic_ring[31] = '\0';
  strncpy(config_.mqtt_topic_control, "home/bell/control", 31);
  config_.mqtt_topic_control[31] = '\0';
...
// Legacy get_mqtt_config() removed - incompatible with old MQTT::MQTTConfig struct
// Use get_config() and map SmartBellConfig fields directly to your MQTT client

void ConfigManager::set_mqtt_broker(const uint8_t* broker_ip, uint16_t port) {
  memcpy(config_.mqtt_broker_ip, broker_ip, 4);
  config_.mqtt_port = port;
}

void ConfigManager::set_mqtt_credentials(const char* client_id, const char* username,
                                         const char* password) {
  strncpy(config_.mqtt_client_id, client_id, 23);
  config_.mqtt_client_id[23] = '\0';
  strncpy(config_.mqtt_username, username, 23);
  config_.mqtt_username[23] = '\0';
  strncpy(config_.mqtt_password, password, 23);
  config_.mqtt_password[23] = '\0';
}

void ConfigManager::set_mqtt_topics(const char* topic_ring, const char* topic_control) {
  strncpy(config_.mqtt_topic_ring, topic_ring, 31);
  config_.mqtt_topic_ring[31] = '\0';
  strncpy(config_.mqtt_topic_control, topic_control, 31);
  config_.mqtt_topic_control[31] = '\0';
}

void ConfigManager::set_dhcp(bool enable) { config_.use_dhcp = enable; }

void ConfigManager::set_static_ip(const uint8_t* ip, const uint8_t* subnet,
...
void ConfigManager::print_config() const {
  ...
  // Print broker IP
  char ip_str[20];
  snprintf(ip_str, 20, "%d.%d.%d.%d:%d", config_.mqtt_broker_ip[0], config_.mqtt_broker_ip[1],
           config_.mqtt_broker_ip[2], config_.mqtt_broker_ip[3], config_.mqtt_port);
  ...
  if (!config_.use_dhcp) {
    ...
    char ip_str[4];
    itoa(config_.static_ip[0], ip_str, 10);
    uart_->send_string(ip_str);
    uart_->send_string(".");
    itoa(config_.static_ip[1], ip_str, 10);
    uart_->send_string(ip_str);
    uart_->send_string(".");
    itoa(config_.static_ip[2], ip_str, 10);
    uart_->send_string(ip_str);
    uart_->send_string(".");
    itoa(config_.static_ip[3], ip_str, 10);
    uart_->send_string(ip_str);
    uart_->send_string("\r\n");
  }
...
bool ConfigManager::parse_set_broker(const char* args) {
  ...
  // Parse: <ip> <port> (e.g., "192.168.1.100 1883")
  int parsed = sscanf(args, "%hu.%hu.%hu.%hu %hu", &ip0, &ip1, &ip2, &ip3, &port);
  if (parsed == 5 && ip0 <= 255 && ip1 <= 255 && ip2 <= 255 && ip3 <= 255) {
    uint8_t broker_ip[4] = {static_cast<uint8_t>(ip0), static_cast<uint8_t>(ip1),
                            static_cast<uint8_t>(ip2), static_cast<uint8_t>(ip3)};
    set_mqtt_broker(broker_ip, port);
    uart_->send_string("[Config] Broker set to: ");
    char ip_str[20];
    snprintf(ip_str, 20, "%d.%d.%d.%d\r\n", ip0, ip1, ip2, ip3);
    uart_->send_string(ip_str);
    return true;
  }

  uart_->send_string("[Config] Usage: set broker <ip> <port> (e.g., 192.168.1.100 1883)\r\n");
  return false;
}
...
bool ConfigManager::parse_set_ip(const char* args) {
  ...
  int parsed = sscanf(args, "%hhu.%hhu.%hhu.%hhu %hhu.%hhu.%hhu.%hhu %hhu.%hhu.%hhu.%hhu", &ip[0],
                      &ip[1], &ip[2], &ip[3], &subnet[0], &subnet[1], &subnet[2], &subnet[3],
